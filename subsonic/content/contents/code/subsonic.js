...
var SubsonicResolver = Tomahawk.extend(TomahawkResolver, {
    ...
    _encodePassword: function (password) {
        var hex_slice;
        var hex_string = "";
        var padding = ["", "0", "00"];
     ...
     * (version1 > version2) == 1
     */
    _versionCompare: function (version1, version2) {
    ...
    init: function () {
        ...
        this.password = this._encodePassword(userConfig.password);
        this.subsonic_url = userConfig.subsonic_url || "";
        ...
        var ping_url = this._buildBaseUrl("/rest/ping.view");
        Tomahawk.asyncRequest(ping_url, function (xhr) {
            ...
            if (typeof Tomahawk.reportCapabilities == 'function') {
                ...
                if (that._versionCompare(version, that.subsonic_api) >= 0) {
    ...
    configTest: function () {
        Tomahawk.asyncRequest(this._buildBaseUrl("/rest/ping.view"),
            function (xhr) {
                try {
                    var response = JSON.parse(xhr.responseText);
                } catch (e) {
    ...
    _buildBaseUrl: function (subsonic_view) {
    ...
    _parseSongFromAttributes: function (song_attributes) {
        return {
            ...
            url: this._buildBaseUrl("/rest/stream.view") + "&id=" + song_attributes["id"],
            extension: song_attributes["suffix"],
            year: song_attributes["year"]
        };
    },

    _executeSearchQuery: function (qid, search_url, isResolve) {
        var results = [];
        var that = this; // needed so we can reference this from within the lambda

        // Important to recognize this async request is doing a get and the user / password is
        // passed in the search url
        ...
        Tomahawk.asyncRequest(search_url, function (xhr) {
            ...
            if (searchResults) {
                ...
                for (var i = 0; i < searchResults.length; i++) {
                    results.push(that._parseSongFromAttributes(searchResults[i]));
                }
    ...
    _executeArtistsQuery: function (qid, artists_url) {
        var results = [];
    ...
    _executeAlbumsQuery: function (qid, search_url, artist) {
        var results = [];
        ...
        Tomahawk.asyncRequest(search_url, function (xhr) {
            ...
            if (albums instanceof Array) {
                ...
                for (var i = 0; i < albums.length; i++) {
                    if (albums[i].artist.toLowerCase() === artist.toLowerCase())
                    // search2 does partial matches
                    {
    ...
    _executeTracksQuery: function (qid, search_url, artist, album) {
        var results = [];
        var that = this;
            ...
            if (tracks instanceof Array) {
                ...
                for (var i = 0; i < tracks.length; i++) {
                    ...
                    if (tracks[i].artist && artist
                        ...
                        results.push(that._parseSongFromAttributes(tracks[i]));
                    }
                }
                ...
                results.push(that._parseSongFromAttributes(tracks));
            }
    ...
    // The reason i am doing this is because it allows me to get a little more specific with the
    // search since i have the artist, album and title i want to be as specific as possible
    // NOTE: I do use the newer search3.view in the search method below and it will populate each
    // result with the appropriate url.
    resolve: function (qid, artist, album, title) {
        if (!this.user || !this.password || !this.subsonic_url) {
            return {qid: qid, results: []};
        }

        var search_url = this._buildBaseUrl("/rest/search.view") + "&artist=" + artist + "&album="
            + album + "&title=" + title + "&count=1";
        this._executeSearchQuery(qid, search_url, true);
    },
    ...
    search: function (qid, searchString) {
        ...
        var search_url = this._buildBaseUrl("/rest/search3.view") + "&songCount=100&query=\""
            + encodeURIComponent(searchString) + "\"";
        this._executeSearchQuery(qid, search_url, false);
    },
    ...
    artists: function (qid) {
        ...
        var artists_url = this._buildBaseUrl("/rest/getArtists.view");
        this._executeArtistsQuery(qid, artists_url);
    },
    ...
    albums: function (qid, artist) {
        ...
        var search_url = this._buildBaseUrl("/rest/search3.view")
            + "&songCount=0&artistCount=0&albumCount=900" +
            "&query=\"" + encodeURIComponent(artist) + "\"";
        this._executeAlbumsQuery(qid, search_url, artist);
    },
    ...
    tracks: function (qid, artist, album) {
        ...
        var search_url = this._buildBaseUrl("/rest/search.view") +
            "&artist=\"" + encodeURIComponent(artist) +
            "\"&album=\"" + encodeURIComponent(album) + "\"&count=200";

        this._executeTracksQuery(qid, search_url, artist, album);
    },
