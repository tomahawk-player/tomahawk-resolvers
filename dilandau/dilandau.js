...
var DilandauResolver = Tomahawk.extend(TomahawkResolver, {
    ...
    parseISODuration: function (duration) {
        var splitDurationRegex_ = new RegExp('^(-)?P(?:(\\d+)Y)?(?:(\\d+)M)?(?:(\\d+)D)?' + '(T(?:(\\d+)H)?(?:(\\d+)M)?(?:(\\d+(?:\\.\\d+)?)S)?)?$');
        ...
        return negative ? (-days * 24 * 3600 - hours * 3600 - minutes * 60 - seconds) : (days * 24 * 3600 + hours * 3600 + minutes * 60 + seconds);
                ...
                for (var i = 0; i < matches.length; i++) {
                    ...
                    if (!that.settings.strictMatch || (dTitleLower.indexOf(artist.toLowerCase()) !== -1 && dTitleLower.indexOf(title.toLowerCase()) !== -1)) {
                        ...
                        (function(result) {
                            ...
                            xhr.open("HEAD", result.url, true);
                            xhr.onreadystatechange = function() {
                                if (xhr.readyState !== 4)
                                    return;

//                                 Tomahawk.log("Got result check for mp3, count: " + count + " and total: " + total);
                                if (xhr.status === 200 || xhr.status === 206) { // We accept 200 (OK) and 206 (Partial Content)
                                    results.push(result)
                                } else {
                                    Tomahawk.log("Found 404'ed mp3 url, ignoring: " + result.url);
//                                 Tomahawk.log("GOT HEADER CHECK FOR: " + result.url);
//                                 Tomahawk.log("XHR STATE CHANGED: " + xhr.readyState);
//                                 Tomahawk.log("XHR STATUS: " + xhr.status);
                                }

                                count = count + 1;

                                if (count == limit) {
                                    Tomahawk.log("Found all playable urls, sending results to Tomahawk!");
...
//                             Tomahawk.log("Sending check for: " + result.url);
                            xhr.send(null);
                        })(result);
                    } else {
                        count = count + 1;
